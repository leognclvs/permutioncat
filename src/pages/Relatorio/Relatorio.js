...
import Relatorio from '../../components/Header/relatorio.svg';
...
function Order() {
  ...
  useEffect(() => {
    const fetchAvailableCats = async () => {
      try {
        const response = await axios.get('https://permutioncat.fly.dev/cats-relatorio/');
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.post('https://permutioncat.fly.dev/relatorio/', formData);
      setPopupVisible(true); // Exibe o popup
      setTimeout(() => {
        setPopupVisible(false);
        navigate('/'); // Redireciona para o menu Ind após 5 segundos
      }, 2500);
    } catch (error) {
  ...
  return (
    <div className="form-rel">
      <div>
        <img className="order" src={Relatorio} alt="instalacao" />
      </div>
      <div className="startext">Relatório Interno</div>
